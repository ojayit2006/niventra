        ...
        <div className="space-y-20 sm:space-y-24">
          {qualifications.items.map((item, i) => (
            <Reveal key={item.text} delay={Math.min(i * 0.06, 0.3)}>
              <div
                className={`grid gap-6 border-b border-line pb-16 last:border-0 last:pb-0 sm:items-start sm:gap-8 ${
                  item.images.length > 0 ? "sm:grid-cols-[auto_1fr_1.3fr]" : "sm:grid-cols-[auto_1fr]"
                }`}
              >
                ...
                <div>
                  {item.images.length === 0 && (
                    <p className="font-mono-label mb-2 text-xs tracking-[0.02em] text-slate-light">
                  ...
                  <p
                    className={`font-display font-semibold leading-snug text-ink ${
                      item.images.length > 0
                        ? "max-w-md text-xl sm:text-2xl"
                        : "max-w-none text-2xl sm:text-4xl"
                    }`}
                  >
                    {item.text}
                  </p>
